...
async def receive_ca_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cached:
        ...
        async with _api_semaphore:
            ...
            # Fetch socials + holders
            _socials, _holders = await asyncio.gather(
                dexscreener_api.fetch_pump_socials(ca),
                dexscreener_api.fetch_holders(ca),
            )
            pump_socials = _socials
            _holders_count = _holders
            ...
            # Salva in cache
            _set_cached_preview(ca, token_info, {}, {}, pump_socials)
