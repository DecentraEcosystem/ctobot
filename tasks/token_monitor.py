...
from formatters.message_formatter import format_token_message, format_gain_alert
...
class TrackedToken:
    ...
    dex_boost_posted: bool = False
    dex_ads_posted: bool = False
    dex_cto_posted: bool = False
...
class TokenMonitor:
    ...
    async def _process_token_candidates(self, tokens: list):
        """
        CTObot: posta TUTTI i nuovi CTO Solana senza filtri.
    ...
    async def _check_dex_boosts(self):
        ...
        try:
            ...
            for mint, boost_data in sol_boosts.items():
                ...
                if not tracked or tracked.dex_boost_posted:
                    continue
    ...
    async def _post_dex_boost_alert(self, tracked, amount: int = 0, total_amount: int = 0) -> bool:
        try:
            if not self._bot_username:
                bot_info = await self.bot.get_me()
                self._bot_username = bot_info.username
            ...
            original_post_link = f"https://t.me/{channel_username}/{tracked.message_id}" if channel_username else None
            symbol_display = f"<a href='{original_post_link}'><b>{symbol}</b></a>" if original_post_link else f"<b>{symbol}</b>"
            ...
            logo_bytes = await self._normalize_logo(tracked.logo_url, symbol)
            await self.bot.send_photo(chat_id=self.channel_id, photo=logo_bytes, caption=msg, reply_markup=keyboard, parse_mode='HTML')
            return True
        except Exception as e:
    ...
    async def _check_dex_ads(self):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get("https://api.dexscreener.com/ads/latest/v1", timeout=aiohttp.ClientTimeout(total=10)) as resp:
                    if resp.status != 200:
                        return
                    data = await resp.json()
            if not isinstance(data, list):
    ...
    async def _post_dex_ads_alert(self, tracked, ad_data: dict = {}) -> bool:
        try:
            if not self._bot_username:
                bot_info = await self.bot.get_me()
                self._bot_username = bot_info.username
            ...
            if ad_type:     msg += f"🎯 Type: <b>{ad_type}</b>\n"
            if duration:    msg += f"⏱ Duration: <b>{duration}h</b>\n"
            if impressions: msg += f"👁 Impressions: <b>{impressions:,}</b>\n"
            msg += "\n"
            msg += f"<a href='https://dexscreener.com/solana/{mint}'><b>📊 Dexscreener</b></a>  |  "
            msg += f"<a href='https://pump.fun/{mint}'><b>🎯 Pump.fun</b></a>"
            keyboard = _make_token_keyboard(mint, self._bot_username)
